import React, { useState, useEffect } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { useGoogleLogin } from '@react-oauth/google';
import { Sparkles, LayoutGrid, Bot, Clock, Eye, EyeOff } from 'lucide-react';

// ÖNEMLİ: Yandex Developer panelinden aldığın Client ID'yi buraya yapıştır!
const YANDEX_CLIENT_ID = "de52adf1963248a2b0a4b34a2e6e0b74";

const LoginPage = () => {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  const [error, setError] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  
  const navigate = useNavigate();

  const bgMeshStyle = {
    backgroundColor: '#f6f7f8',
    backgroundImage: `
      radial-gradient(at 0% 0%, hsla(203, 85%, 90%, 1) 0, transparent 50%), 
      radial-gradient(at 100% 0%, hsla(203, 85%, 85%, 1) 0, transparent 50%), 
      radial-gradient(at 100% 100%, hsla(203, 85%, 95%, 1) 0, transparent 50%), 
      radial-gradient(at 0% 100%, hsla(203, 85%, 92%, 1) 0, transparent 50%)
    `
  };

  const glassClass = "bg-white/70 backdrop-blur-md border border-white/40 shadow-xl";

  // --- YANDEX DÖNÜŞÜNÜ (CALLBACK) YAKALAMA ---
  useEffect(() => {
    const hash = window.location.hash;
    if (hash && hash.includes('access_token=')) {
      const token = new URLSearchParams(hash.substring(1)).get('access_token');
      
      if (token) {
        setIsLoading(true); // Yükleniyor animasyonunu başlat
        
        // Backend'deki giriş/kayıt olma API'sine tokeni yolluyoruz
        fetch('http://localhost:5000/api/auth/yandex', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ access_token: token })
        })
        .then(res => res.json().then(data => ({status: res.status, data})))
        .then(({status, data}) => {
          if (status === 200) {
            // Başarılı giriş veya otomatik kayıt!
            localStorage.setItem('token', data.access_token);
            localStorage.setItem('user', JSON.stringify(data.user_info));
            navigate('/mail'); // Direkt maillere ışınla
          } else if (status === 404) {
            setError('Hesap bulunamadı.');
          } else {
            setError(data.detail || 'Yandex ile giriş başarısız oldu.');
          }
        })
        .catch(() => setError('Sunucu bağlantı hatası.'))
        .finally(() => {
          setIsLoading(false);
          // URL'deki uzun token yazısını temizle ki sayfa temiz görünsün
          window.history.replaceState(null, '', '/login'); 
        });
      }
    }
  }, [navigate]);

  // --- YANDEX İLE GİRİŞ BUTONUNA TIKLANINCA ---
  const handleYandexLogin = () => {
    const YANDEX_CLIENT_ID = "de52adf1963248a2b0a4b34a2e6e0b74"; // Senin ID'n
    const redirectUri = 'http://localhost:5173/login'; // Giriş sayfası için login'e dönsün
    window.location.href = `https://oauth.yandex.com/authorize?response_type=token&client_id=${YANDEX_CLIENT_ID}&redirect_uri=${redirectUri}`;
  };

  // --- GOOGLE İLE GİRİŞ ---
  const loginWithGoogle = useGoogleLogin({
    flow: 'auth-code',
    onSuccess: async (codeResponse) => {
      setError('');
      setIsLoading(true);
      try {
        const response = await fetch('http://localhost:5000/api/auth/google', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ code: codeResponse.code }),
        });

        const data = await response.json();

        if (response.ok) {
          localStorage.setItem('token', data.access_token);
          localStorage.setItem('user', JSON.stringify(data.user_info));
          navigate('/mail');
        } else if (response.status === 404) {
          setError('Hesap bulunamadı. Kayıt sayfasına yönlendiriliyorsunuz...');
          setTimeout(() => navigate('/register'), 2000);
        } else {
          setError(data.detail || 'Google ile giriş başarısız oldu.');
        }
      } catch (err) {
        setError('Sunucuya bağlanırken bir hata oluştu.');
      } finally {
        setIsLoading(false);
      }
    },
    onError: () => setError('Google ile giriş iptal edildi veya başarısız oldu.')
  });

  // --- NORMAL GİRİŞ ---
  const handleLogin = async (e) => {
    e.preventDefault();
    setError('');
    setIsLoading(true);

    try {
      const response = await fetch('http://localhost:5000/api/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password }),
      });

      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.detail || 'Giriş başarısız. Lütfen bilgilerinizi kontrol edin.');
      }

      localStorage.setItem('token', data.access_token);
      localStorage.setItem('user', JSON.stringify(data.user_info));
      navigate('/mail'); 

    } catch (err) {
      setError(err.message);
    } finally {
      setIsLoading(false);
    }
  };

  return (
    <div className="font-display bg-[#f6f7f8] min-h-screen text-slate-900 relative overflow-x-hidden selection:bg-[#2b9dee]/30">
      
      {/* Arka Plan Renk Geçişleri (Mesh Gradient) */}
      <div className="fixed top-[-10%] right-[-5%] w-[40vw] h-[40vw] rounded-full bg-[#2b9dee]/15 blur-[120px] pointer-events-none -z-10"></div>
      <div className="fixed bottom-[-10%] left-[-5%] w-[30vw] h-[30vw] rounded-full bg-blue-300/15 blur-[100px] pointer-events-none -z-10"></div>
      
      <div className="relative flex min-h-screen w-full flex-col">
        
        {/* Navbar */}
        <header className="flex items-center justify-between whitespace-nowrap px-6 py-6 w-full max-w-[1300px] mx-auto z-10 shrink-0">
          <div className="flex items-center gap-2.5 text-[#2b9dee]">
            <div className="w-8 h-8 bg-[#2b9dee] rounded-lg flex items-center justify-center text-white shadow-md">
              <span className="material-symbols-outlined text-[20px]">auto_awesome</span>
            </div>
            <h2 className="text-slate-900 text-xl font-black leading-tight tracking-tight">AİA</h2>
          </div>
          <div className="flex items-center gap-8">
            <nav className="hidden md:flex items-center gap-7">
              <a className="text-slate-600 hover:text-[#2b9dee] text-sm font-medium transition-colors" href="/#urun">Özellikler</a>
              <a className="text-slate-600 hover:text-[#2b9dee] text-sm font-medium transition-colors" href="/#fiyatlandirma">Fiyatlandırma</a>
              <a className="text-slate-600 hover:text-[#2b9dee] text-sm font-medium transition-colors" href="/#guvenlik">Destek</a>
            </nav>
            <button 
              onClick={() => navigate('/register')} 
              className="flex min-w-[100px] items-center justify-center rounded-full h-10 px-5 border border-[#2b9dee]/20 bg-white/50 text-[#2b9dee] text-sm font-bold hover:bg-white transition-all shadow-sm"
            >
              Kayıt Ol
            </button>
          </div>
        </header>

        {/* Ana İçerik */}
        <main className="flex-1 flex items-center justify-center p-4 lg:p-10 z-10 w-full">
          <div className="w-full max-w-[1200px] flex flex-row flex-nowrap items-center justify-between gap-12">
            
            {/* Sol Taraf - Metinler ve Özellik Kartları */}
            <div className="flex-1 flex flex-col gap-8 min-w-[500px]">
              <div className="flex flex-col gap-4">
                <span className="inline-flex items-center px-3 py-1 rounded-full bg-[#2b9dee]/10 text-[#2b9dee] text-xs font-bold uppercase tracking-wider w-fit">
                  Yapay Zeka Devrimi
                </span>
                <h1 className="text-slate-900 text-4xl lg:text-5xl xl:text-[54px] font-black leading-[1.05] tracking-tight">
                  AİA - Geleceğin<br/>E-posta Yönetimi
                </h1>
                <p className="text-slate-600 text-base lg:text-[17px] max-w-[500px] leading-relaxed font-medium mt-1">
                  Gelen kutunuzu yapay zeka ile yönetin. Akıllı asistanınız sayesinde iş akışınızı hızlandırın ve zaman kazanın.
                </p>
              </div>

              <div className="flex flex-col gap-4 w-full max-w-[500px]">
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-white/60 backdrop-blur-md shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 hover:shadow-md transition-shadow">
                  <div className="w-12 h-12 rounded-xl bg-[#e8f2ff] flex items-center justify-center text-[#4a90e2] shrink-0">
                    <span className="material-symbols-outlined">category</span>
                  </div>
                  <div className="pt-0.5">
                    <h3 className="text-slate-900 font-bold text-[17px] mb-0.5">Akıllı Kategorizasyon</h3>
                    <p className="text-slate-500 text-[14px] font-medium leading-relaxed">E-postalarınız içeriklerine göre otomatik olarak klasörlenir ve önceliklendirilir.</p>
                  </div>
                </div>
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-white/60 backdrop-blur-md shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 hover:shadow-md transition-shadow">
                  <div className="w-12 h-12 rounded-xl bg-[#f4e8ff] flex items-center justify-center text-[#9013fe] shrink-0">
                    <span className="material-symbols-outlined">smart_toy</span>
                  </div>
                  <div className="pt-0.5">
                    <h3 className="text-slate-900 font-bold text-[17px] mb-0.5">Yapay Zeka Yanıtları</h3>
                    <p className="text-slate-500 text-[14px] font-medium leading-relaxed">Gelen mesajlara bağlamı anlayarak tek tıkla profesyonel yanıt taslakları oluşturun.</p>
                  </div>
                </div>
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-white/60 backdrop-blur-md shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 hover:shadow-md transition-shadow">
                  <div className="w-12 h-12 rounded-xl bg-[#e8fcf0] flex items-center justify-center text-[#2ecc71] shrink-0">
                    <span className="material-symbols-outlined">schedule_send</span>
                  </div>
                  <div className="pt-0.5">
                    <h3 className="text-slate-900 font-bold text-[17px] mb-0.5">Zamanlanmış Gönderim</h3>
                    <p className="text-slate-500 text-[14px] font-medium leading-relaxed">İletilerinizin alıcıya en uygun olduğu saatlerde ulaşmasını otomatik planlayın.</p>
                  </div>
                </div>
              </div>
            </div>

            {/* Sağ Taraf - Giriş Formu */}
            <div className="w-[450px] shrink-0">
              <div className="w-full rounded-[2rem] bg-white/70 backdrop-blur-xl p-8 lg:p-10 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.08)] border border-white/80">
                <div className="mb-8">
                  <h2 className="text-[28px] font-extrabold text-slate-900 mb-1">Giriş Yap</h2>
                  <p className="text-slate-500 font-medium text-[15px]">Hesabınıza erişmek için bilgilerinizi girin.</p>
                </div>

                {error && <div className="mb-5 p-3 bg-red-50 border border-red-100 text-red-600 rounded-lg text-sm font-semibold">{error}</div>}

                {/* Logolar */}
                <div className="grid grid-cols-2 gap-4 mb-7">
                  <button 
                    type="button" 
                    onClick={() => loginWithGoogle()}
                    className="flex items-center justify-center gap-2.5 rounded-2xl border border-slate-200/80 bg-white/80 py-3 hover:bg-white transition-all shadow-sm"
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
                    <span className="text-[14px] font-bold text-slate-700">Google</span>
                  </button>

                  <button 
  type="button" 
  onClick={handleYandexLogin}
  className="flex items-center justify-center gap-2.5 rounded-2xl border border-slate-200/80 bg-white/80 py-3 hover:bg-white transition-all shadow-sm"
>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M15.428 21.643h-4.04V13.8l-4.59-11.44h4.316l2.36 6.84 2.26-6.84h4.08l-4.386 11.13v8.153z" fill="#FC3F1D"/>
                    </svg>
                    <span className="text-[14px] font-bold text-slate-700">Yandex</span>
                  </button>
                </div>

                <div className="relative my-7">
                  <div className="absolute inset-0 flex items-center">
                    <div className="w-full border-t border-slate-200/80"></div>
                  </div>
                  <div className="relative flex justify-center text-[11px] font-bold uppercase tracking-widest">
                    <span className="bg-white/80 backdrop-blur-sm px-3 text-slate-400 rounded-full">VEYA E-POSTA İLE</span>
                  </div>
                </div>

                {/* Form */}
                <form className="space-y-4" onSubmit={handleLogin}>
                  
                  <div className="space-y-1.5">
                    <label className="text-[13px] font-bold text-slate-800 ml-1">E-posta</label>
                    <input 
                      type="email"
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      className="w-full rounded-[14px] border-slate-200/80 bg-white/60 px-4 py-3.5 text-[14px] focus:border-[#2b9dee] focus:ring-2 focus:ring-[#2b9dee]/20 transition-all outline-none placeholder:text-slate-400" 
                      placeholder="ornek@aia.com" 
                      required
                    />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[13px] font-bold text-slate-800 ml-1">Şifre</label>
                    <div className="relative">
                      <input 
                        type={showPassword ? "text" : "password"}
                        value={password}
                        onChange={(e) => setPassword(e.target.value)}
                        className="w-full rounded-[14px] border-slate-200/80 bg-white/60 px-4 py-3.5 pr-10 text-[14px] focus:border-[#2b9dee] focus:ring-2 focus:ring-[#2b9dee]/20 transition-all outline-none placeholder:text-slate-400" 
                        placeholder="••••••••" 
                        required
                      />
                      <button 
                        type="button" 
                        onClick={() => setShowPassword(!showPassword)}
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"
                      >
                        <span className="material-symbols-outlined text-[20px]">
                          {showPassword ? "visibility_off" : "visibility"}
                        </span>
                      </button>
                    </div>
                  </div>

                  <div className="flex items-center justify-between py-2">
                    <div className="flex items-center gap-2.5 ml-1">
                      <input 
                        type="checkbox" 
                        id="remember" 
                        className="rounded border-slate-300 text-[#2b9dee] focus:ring-[#2b9dee]/20 w-[16px] h-[16px] cursor-pointer" 
                      />
                      <label className="text-[13px] text-slate-500 cursor-pointer font-medium" htmlFor="remember">
                        Beni Hatırla
                      </label>
                    </div>
                    <a className="text-[13px] font-bold text-[#2b9dee] hover:underline" href="#">Şifremi Unuttum</a>
                  </div>

                  <button 
                    type="submit" 
                    disabled={isLoading}
                    className={`w-full rounded-2xl bg-[#2b9dee] py-4 text-white font-bold shadow-lg shadow-blue-500/30 hover:bg-[#208ad6] hover:-translate-y-0.5 transition-all active:scale-[0.98] mt-2 ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
                  >
                    {isLoading ? 'Giriş Yapılıyor...' : 'Giriş Yap'}
                  </button>
                </form>

                <p className="mt-7 text-center text-[14px] text-slate-500 font-medium">
                  Henüz hesabınız yok mu? 
                  <Link className="font-bold text-[#2b9dee] hover:underline ml-1" to="/register">Yeni Hesap Oluştur</Link>
                </p>
              </div>
            </div>

          </div>
        </main>
      </div>
    </div>
  );
};

export default LoginPage;